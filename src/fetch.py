...
def whisper_pass(workdir, whisper_model, lang):
    ...
    if not has("mlx_whisper"):
        print(
            f"!! {len(pending)} video(s) have no captions and mlx_whisper is not installed — "
            f"skipping. Install with: uv sync --extra whisper",
            file=sys.stderr,
        )
        return
    for info in pending:
...
def main():
    ap = argparse.ArgumentParser(
        description="Fetch YouTube transcripts (auto-detect captions vs Whisper)."
    )
    ap.add_argument(
        "url", help="video, playlist, or channel URL, e.g. https://www.youtube.com/@handle/videos"
    )
    ...
    ap.add_argument(
        "--no-whisper", action="store_true", help="captions only; skip the audio fallback"
    )
    a = ap.parse_args()
